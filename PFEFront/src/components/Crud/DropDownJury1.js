...
const DropDownJury1 = (props) => {
  ...
  const [jury1,setJury1] = useLocalStorage("jury1",{})
    ...
    return(
        <div className='dropdown'>
              ...
              {selectedItem ? encadrants.find(item => item.nom == selectedItem).nom : "Selectionner le jury 1"}
              <i className={`fa fa-chevron-right icon ${isOpen && "open"}`}></i>
            </div>
            
            <div className={`dropdown-body ${isOpen && 'open'}`}>
            {encadrants.map(item => (
              <div className="dropdown-item" onClick={e => {handleItemClick(e.target.id,item); setJury1(item); setFullname(item.nom+" "+item.prenom);setOP(false)}} id={item.nom} item={item}>
